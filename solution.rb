...
possible_paths.each do |path_array|
	...
	# Compare north
	if y > 0 && !array[y-1][x].nil?
		if array[y-1][x] < coord
	...
	# Compare east
	if x < dim[0] - 1 && !array[y][x+1].nil?
		if array[y][x+1] < coord
			new_path = path_array.dup
			new_path << [x+1,y]
			possible_paths << new_path
		end
...
#p max_routes = possible_path.max
